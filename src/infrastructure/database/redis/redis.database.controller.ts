...
export class RedisDatabaseController {
  ...
  @ApiResponse({ status: 200, description: 'Mensajes de chat recuperados de caché' })
  getChatMessages(
  ...
  @ApiResponse({ status: 200, description: 'Información de sala recuperada de caché' })
  getChatRoom(@Param('roomId') roomId: string) {
    return this.databaseService.getChatRoom(roomId);
  }

  @Get('card/:cardId/location')
  @ApiOperation({ summary: 'Obtiene la ubicación de una tarjeta desde la caché' })
  @ApiResponse({ status: 200, description: 'Ubicación de tarjeta recuperada de caché' })
