...
export class AuthenticationResultDto {
  ...
  @ApiProperty({ example: 900, description: 'Access token validity period in seconds' })
  expires_in: number;

  @ApiProperty({ example: 'Bearer', description: 'Token type' })
  token_type: string;

  @ApiProperty({ type: () => EmployeeBasicDto, description: 'Basic information of the authenticated employee', required: false })
