...
export class GetVisitorsBySupplierUseCase {
  ...
  async execute(supplierId: string): Promise<Visitor[]> {
    this.logger.log(`Attempting to fetch visitors for supplier id: ${supplierId}`, { supplierId });
    ...
    if (!supplier) {
      this.logger.warn(`Supplier not found when attempting to fetch its visitors: ${supplierId}`, { supplierId });
    ...
    this.logger.log(`Successfully fetched ${visitors.length} visitors for supplier id: ${supplierId}`, {
      supplierId,
