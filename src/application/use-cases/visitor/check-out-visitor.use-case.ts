...
export class CheckOutVisitorUseCase {
  ...
  async execute(id: string): Promise<Visitor> {
    this.logger.log(`Attempting to check out visitor with id: ${id}`, undefined, undefined, { visitorId: id });
    this.logger.log(`Visitor check-out initiated`, undefined, undefined, { visitorId: id });
    ...
    if (!visitor) {
      this.logger.warn(`Visitor not found for check-out with id: ${id}`, undefined, undefined, { visitorId: id });
      throw new NotFoundException(`Visitor with ID "${id}" not found`);
    }

    if (visitor.state === 'completado') {
      this.logger.warn(`Visitor already checked out: ${id}`, undefined, undefined, { visitorId: id, currentState: visitor.state });
    ...
    if (!visitor.appointments || visitor.appointments.length === 0) {
      this.logger.warn(`No appointments found for visitor during check-out: ${id}`, undefined, undefined, { visitorId: id });
    ...
    if (!appointmentToUpdate) {
        this.logger.error('Associated appointment not found during checkout, though listed under visitor.', undefined, undefined, { visitorId: id, appointmentId: visitor.appointments[0].id });
    ...
    if (!appointmentToUpdate.check_in_time) {
      this.logger.warn(`Visitor has not checked in for this appointment`, undefined, undefined, { visitorId: id, appointmentId: appointmentToUpdate.id });
    ...
    this.logger.log('Appointment updated to "completado" for check-out', undefined, undefined, { visitorId: id, appointmentId: appointmentToUpdate.id });
    ...
    this.logger.log('Visitor state updated to "completado" for check-out', undefined, undefined, { visitorId: updatedVisitor.id });
    ...
    if (visitor.card && visitor.card.id) { // Check if visitor.card and visitor.card.id exist
      try {
        ...
        this.logger.log('Card unassigned from visitor during check-out', undefined, undefined, { visitorId: id, cardId: visitor.card.id });
      } catch (error) {
        this.logger.warn(`Failed to unassign card during check-out for visitor: ${id}. This will not fail the checkout.`, undefined, undefined, {
          visitorId: id,
    ...
    try {
      ...
      this.logger.log('Visitor checkout email dispatch requested successfully', undefined, undefined, { visitorId: updatedVisitor.id, email: updatedVisitor.email });
    } catch (error) {
      this.logger.warn(`Failed to send visitor checkout email for: ${updatedVisitor.id}. This will not fail the checkout.`, undefined, undefined, {
