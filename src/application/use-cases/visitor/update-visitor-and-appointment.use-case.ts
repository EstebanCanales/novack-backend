...
import { UpdateVisitorDto } from '../../../dtos/visitor/update-visitor.dto'; // Assuming DTO path
...
export class UpdateVisitorAndAppointmentUseCase {
  ...
  private validateDates(check_in_time: Date, check_out_time?: Date): void {
    if (check_out_time) {
      if (new Date(check_out_time) <= new Date(check_in_time)) {
        this.logger.warn('Validation failed: Check-out time must be after check-in time during update.', { check_in_time, check_out_time });
  ...
  async execute(id: string, updateVisitorDto: UpdateVisitorDto): Promise<Visitor> {
    this.logger.log('Attempting to update visitor and associated appointment', { visitorId: id, updateData: updateVisitorDto });
    ...
    if (!visitor) {
      this.logger.warn(`Visitor not found for update with id: ${id}`, { visitorId: id });
    ...
    if (!visitor.appointments || visitor.appointments.length === 0) {
      this.logger.warn('Visitor has no associated appointments to update', { visitorId: id });
    ...
    if (!appointmentToUpdate) {
        this.logger.error('Associated appointment not found during update despite being listed under visitor.', { visitorId: id, appointmentId: visitor.appointments[0].id });
    ...
    if (updateVisitorDto.supplier_id) {
      ...
      if (!supplier) {
        this.logger.warn('Supplier not found during visitor update', { supplierId: updateVisitorDto.supplier_id, visitorId: id });
    ...
    this.logger.log('Visitor entity updated successfully', { visitorId: updatedVisitor.id });
    ...
    this.logger.log('Associated appointment updated successfully', { appointmentId: appointmentToUpdate.id, visitorId: id });
    ...
    if (!finalVisitor) {
        this.logger.error('Failed to re-fetch visitor after update, though update operations were successful.', { visitorId: id });
        throw new NotFoundException(`Visitor with ID "${id}" could not be found after update operations.`);
    }
    this.logger.log('Successfully updated visitor and appointment details.', { visitorId: finalVisitor.id });
    return finalVisitor;
  }
}
