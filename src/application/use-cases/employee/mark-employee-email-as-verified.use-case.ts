...
export class MarkEmployeeEmailAsVerifiedUseCase {
  ...
  async execute(employeeId: string): Promise<Employee> {
    this.logger.log(`Attempting to mark email as verified for employee id: ${employeeId}`, undefined, JSON.stringify({ employeeId }));
    ...
    if (!employee) {
      this.logger.warn(`Employee not found when attempting to mark email as verified: ${employeeId}`, undefined, JSON.stringify({ employeeId }));
    ...
    if (!employee.credentials) {
        this.logger.error(`Employee credentials not found for employee: ${employeeId}. Cannot verify email.`, undefined, JSON.stringify({ employeeId }));
    ...
    if (employee.credentials.is_email_verified) {
        this.logger.log(`Email is already verified for employee: ${employeeId}. No action taken.`, undefined, JSON.stringify({ employeeId }));
    ...
    this.logger.log(`Email marked as verified successfully for employee id: ${employeeId}`, undefined, JSON.stringify({ employeeId }));
    ...
    if (!updatedEmployee) {
      ...
      this.logger.error(`Critical: Failed to re-fetch employee after marking email verified: ${employeeId}. This may indicate a data consistency issue.`, undefined, JSON.stringify({ employeeId }));
      throw new NotFoundException(`Employee with ID "${employeeId}" could not be found after verification update.`);
    }

    this.logger.log(`Successfully re-fetched employee after email verification: ${employeeId}`, undefined, JSON.stringify({ employeeId, isEmailVerified: updatedEmployee.credentials?.is_email_verified }));
