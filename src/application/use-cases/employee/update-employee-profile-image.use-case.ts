...
export class UpdateEmployeeProfileImageUseCase {
  ...
  async execute(id: string, imageUrl: string): Promise<Employee> {
    this.logger.log(`Attempting to update profile image URL for employee id: ${id}`, undefined, JSON.stringify({
      employeeId: id,
    ...
    if (!employee) {
      this.logger.warn(`Employee not found for profile image update with id: ${id}`, undefined, JSON.stringify({ employeeId: id }));
    ...
    this.logger.log(`Successfully updated profile image URL in repository for employee id: ${id}`, undefined, JSON.stringify({
      employeeId: id,
    ...
    if (!finalUpdatedEmployee) {
        ...
        this.logger.error('Critical: Failed to re-fetch employee after profile image URL update. Data inconsistency possible.', undefined, JSON.stringify({ employeeId: id}));
        throw new NotFoundException(`Employee with ID "${id}" could not be found after the update operation.`);
    }

    this.logger.log(`Re-fetched employee after profile image update for id: ${id}`, undefined, JSON.stringify({ employeeId: id }));
