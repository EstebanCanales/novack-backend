...
export class DeleteEmployeeUseCase {
  ...
  async execute(id: string): Promise<void> {
    this.logger.log(`Attempting to delete employee with id: ${id}`, { employeeId: id });
    ...
    if (!employee) {
      this.logger.warn(`Employee not found for deletion with id: ${id}`, { employeeId: id });
    ...
    //   this.logger.warn(`Attempt to delete last admin employee for supplier failed`, { employeeId: id, supplierId: employee.supplier_id });
    ...
    this.logger.log(`Successfully deleted employee with id: ${id}`, { employeeId: id });
  }
