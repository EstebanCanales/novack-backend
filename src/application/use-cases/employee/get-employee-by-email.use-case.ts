...
export class GetEmployeeByEmailUseCase {
  ...
  async execute(email: string): Promise<Employee | null> {
    ...
    this.logger.log(`Attempting to fetch employee with email: ${email}`, { email });
    ...
    if (!employee) {
      this.logger.log(`Employee not found with email: ${email}. Returning null as per contract.`, { email });
    ...
    this.logger.log(`Successfully fetched employee with email: ${email}`, {
