...
async function testS3Connection() {
  ...
  try {
    ...
    console.log(response.Buckets?.map(bucket => bucket.Name).join(', ') || 'No hay buckets disponibles');
...
testS3Connection()
  .then(success => {
    if (success) {
      ...
      console.log('\nPosibles soluciones:');
      console.log('1. Verifica que AWS_ACCESS_KEY_ID y AWS_SECRET_ACCESS_KEY estén configurados en .env');
